...
//   }
// }
// export default PGAdminInstructionPage;
...
const PGAdminInstructionPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!selectedExam) errors.examId = 'required';
    if (!instructionHeading) errors.instructionHeading = 'required';
  ...
  const handleUpload = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        await axios.post(`${BASE_URL}/InstructionCreation/InstructionsUpdate`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
  
        document.getElementById("uploadInputFile_ovl_upload_file").value = "";
        alert("File uploaded successfully!");
        window.location.reload();
      } catch (error) {
        console.error("Error uploading file:", error.response);
            ...
            <div className="instruction_input_contant examSubjects_-contant">
              ...
              <div className="inst">
                ...
                <input
                  ...
                  onChange={(e) => handleFileUpload(e.target.files)}
                />
                {formErrors.file && <span className="error-message"><i className="fa-solid fa-circle"></i>{formErrors.file}</span>}
