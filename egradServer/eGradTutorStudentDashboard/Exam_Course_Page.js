...
router.post('/test_attempt_start_time', (req, res) => {
  const { user_Id, courseCreationId, testCreationTableId,testAttemptStatus } = req.body;

  // Execute the SQL query with the provided parameters and current timestamp
  const sql = 'INSERT INTO test_attempt_start_time (user_Id, courseCreationId, testCreationTableId, studentTestStartTime,testAttemptStatus) VALUES (?, ?, ?,NOW(),?)';
  db.query(sql, [user_Id, courseCreationId, testCreationTableId,testAttemptStatus], (err, result) => {
    if (err) {
...
router.get("/purchasedCourses/:userId", async (req, res) => {
  ...
  try {
    ...
    GROUP_CONCAT(DISTINCT s.subjectName) AS subjectNames,
    t.topicName,
    COUNT(DISTINCT tct.testCreationTableId) AS totalTests 
FROM
...
GROUP BY
    ...
    results.forEach((result) => {
      ...
      const courseId = result.courseCreationId;
      const portalId = result.Portale_Id;
      ...
      const properties = {
        ...
        // Add variable for Portale_Id 4
        customName: portalId === 4 ? "OTS,OVL,PQB" : null,
        ...(portalId === 4 && { topicName: `(TopicName:${result.topicName})` }),
        // ...(portalId === 4 && { topicName: result.topicName }),
      };

      // Add topicName if portalId is 4
      // if (portalId === 4) {
      //   properties.topicName = result.topicName;
      // }
