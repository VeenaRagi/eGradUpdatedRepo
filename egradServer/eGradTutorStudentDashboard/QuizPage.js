...
router.get("/PG_QuestionOptions/:testCreationTableId/:subjectId/:sectionId/:userId", async (req, res) => {
  const { testCreationTableId, subjectId, sectionId, userId } = req.params;
  ...
  try {
    const [rows] = await db.query(
      ...
      FROM 
        ...
        LEFT OUTER JOIN test_creation_table tct ON doc.testCreationTableId = tct.testCreationTableId
        LEFT OUTER JOIN log l ON ur.user_Id = l.user_Id AND ur.user_Id = l.user_Id
        LEFT OUTER JOIN subjects subj ON doc.subjectId = subj.subjectId
        LEFT OUTER JOIN sections sect ON doc.sectionId = sect.sectionId
      WHERE 
        doc.testCreationTableId = ? AND
        doc.subjectId = ? AND
        doc.sectionId = ? AND
        (l.user_Id = ? OR ur.user_answer IS NULL)
      ORDER BY 
        doc.subjectId ASC, doc.sectionId ASC, q.question_id ASC, o.option_index ASC;`,
      [testCreationTableId, subjectId, sectionId, userId]
    );
