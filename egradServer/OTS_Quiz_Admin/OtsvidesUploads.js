...
ovl_links ol 
 ...
 LEFT JOIN exams e ON e.examId=cc.examId WHERE e.branchId=1`;
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: "No videos found for the given courseCreationId" });
    }
...
router.delete("/videslink_delete/:OVL_Linke_Id", async (req, res) => {
  const OVL_Linke_Id = req.params.OVL_Linke_Id;

  try {
    await db.query(`DELETE ol.* FROM ovl_links AS ol WHERE OVL_Linke_Id = ?`, [
      OVL_Linke_Id,
    ]);
...
router.delete("/videslink_delete", async (req, res) => {
  ...
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    return res
      .status(400)
      .json({ error: "Invalid or empty IDs array provided" });
  }
...
router.get("/OVL_data", async (req, res) => {
  const query =
    "SELECT ol.OVL_Linke_Id ,ol.Drive_Link,ol.Lectures_name,cc.courseCreationId,cc.courseName FROM ovl_links AS ol LEFT JOIN course_creation_table cc ON cc.courseCreationId =ol.courseCreationId";
  try {
    const [result] = await db.query(query);
    res.json(result);
  } catch (error) {
...
router.put("/videslink_update/:OVL_Linke_Id", async (req, res) => {
  ...
  if (!Drive_Link || !Lectures_name || Lecture_order === undefined) {
    return res.status(400).json({
      error: "Drive_Link, Lectures_name, and Lecture_order are required.",
    });
  }
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Video link updated successfully." });
  } catch (error) {
...
//===================pg section =====================
router.get("/pgOvlSection", async (req, res) => {
  const query =
    "SELECT * FROM course_creation_table cct LEFT JOIN exams e ON cct.examId=e.examId WHERE cct.Portale_Id=3 AND e.branchId=2 ";
  try {
    const [result] = await db.query(query);
    res.json(result);
  } catch (error) {
...
router.get("/pgOVLVideosDisplay", async (req, res) => {
  // SQL query to select data
  const query = `SELECT 
  ol.OVL_Linke_Id, 
 ol.Lectures_name, 
...
JOIN 
course_creation_table cc ON cc.courseCreationId = ol.courseCreationId
 LEFT JOIN exams e ON e.examId=cc.examId WHERE e.branchId=2;`;
