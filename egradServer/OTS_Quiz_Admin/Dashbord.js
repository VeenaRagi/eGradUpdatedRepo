...
router.get("/test/count", async (req, res) => {
  try {
    const [results, fields] = await db.execute(
      `SELECT
    COUNT(testCreationTableId) AS count
FROM
...
ON
    e.examId = cct.examId WHERE e.branchId=1`
    );
    res.json(results);
  } catch (error) {
...
router.get("/user/count", async (req, res) => {
  try {
    const [results, fields] = await db.execute(
      " SELECT COUNT(studentregistationId) AS count FROM otsstudentregistation WHERE  Branch_Id=1"
    );
    res.json(results);
  } catch (error) {
    console.error("Error fetching course count:", error);
...
router.get("/user/pgUserCount", async (req, res) => {
  try {
    const [results, fields] = await db.execute(
      "SELECT COUNT(studentregistationId) AS count FROM otsstudentregistation WHERE  Branch_Id=2"
    );
    res.json(results);
  } catch (error) {
    console.error("Error fetching course count:", error);
...
LEFT JOIN exams AS e
ON
    e.examId = cct.examId WHERE e.branchId=2`
    );
    res.json(results);
  } catch (error) {
    console.error("Error fetching test count:", error);
