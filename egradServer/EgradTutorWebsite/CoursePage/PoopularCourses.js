...
router.get('/unPurchasedCoursesOnHomePage/:Portale_Id/:Branch_Id',async(req,res)=>{
    const { Portale_Id,Branch_Id } = req.params; 
    const query=`SELECT
    ...
    brn.Branch_Name,
    COUNT(
        DISTINCT tct.testCreationTableId
    ) AS testCount,
...
WHERE
    cct.Portale_Id = ? AND brn.Branch_Id = ?
GROUP BY
    cct.courseCreationId;
`

const [results, fields] = await db.execute(query, [Portale_Id,Branch_Id]);
const organizedData = {};
...
router.get('/unPurchasedCoursesBuyNow/:courseCreationId', async (req, res) => {
  ...
  brn.Branch_Name,
  COUNT(
      DISTINCT CASE WHEN cct.Portale_Id = 3 THEN ol.Drive_Link
  END
) AS videocount,
COUNT(
  DISTINCT CASE WHEN cct.Portale_Id IN(1, 2) THEN tct.testCreationTableId
END
) AS testCount
FROM
  course_creation_table cct
