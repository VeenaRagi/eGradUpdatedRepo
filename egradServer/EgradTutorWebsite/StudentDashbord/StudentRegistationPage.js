...
//     const studentregistationId = result.insertId;
...
//   const buyCoursesValues = [
//     user_Id,
//     studentData.courseCreationId || null,
//     studentregistationId
//   ];

//   await db.execute(buyCoursesSql, buyCoursesValues);
...
router.post('/register', upload.fields([{ name: 'UplodadPhto' }, { name: 'Signature' }, { name: 'Proof' }]), async (req, res) => {
  ...
  try {
    ...
    const studentRollNo = `${currentDate}${String(studentregistationId).padStart(3, '0')}`;

    // Update the student record with the generated roll number
    const updateSql = `
      UPDATE otsstudentregistation
      SET studentrollno = ?
      WHERE studentregistationId = ?`;

    await db.execute(updateSql, [studentRollNo, studentregistationId]);
